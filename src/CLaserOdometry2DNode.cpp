...
CLaserOdometry2DNode::CLaserOdometry2DNode(): Node("CLaserOdometry2DNode")
{
  ...
  {
    initPose_sub = this->create_subscription<mocap4r2_msgs::msg::RigidBodies>(init_pose_from_topic,rclcpp::QoS(rclcpp::KeepLast(1)).best_effort().durability_volatile(),
...
*/
void CLaserOdometry2DNode::initPoseCallBack(const mocap4r2_msgs::msg::RigidBodies::SharedPtr new_initPose)
{
  RCLCPP_INFO(get_logger(), "Initial pose received");
  // Initialize module on first GT pose. Else do Nothing!
  if (!GT_pose_initialized)
  {
    initial_robot_pose.pose.pose = new_initPose->rigidbodies[ROB_ID].pose;
    GT_pose_initialized = true;
    initPose_sub.reset();
  }
}
